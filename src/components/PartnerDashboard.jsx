...
const mockCandidates = [
  {
    ...
    docStatus: 'verified',
    bsnDate: '15.11.2023',
    departureDate: '18.11.2023',
    status: 'ready',
    jobTarget: 'Skladník - Order Picker',
...
export default function PartnerDashboard() {
  ...
  const handleApprove = (id) => {
    ...
    if (selectedCandidate && selectedCandidate.id === id) {
      setSelectedCandidate(prev => ({ ...prev, status: 'active', departureDate: 'Dnes' }));
    }
    alert('Kandidát byl schválen k nástupu a notifikován.');
  };

  const handleExport = () => {
    alert('Exportuji seznam kandidátů do úhledného PDF s logem GoNL...');
  };

  const generatePDFPack = () => {
    alert(`Generuji "PDF Pack" (Profil, OP, BSN) pro ${selectedCandidate.name}.`);
  };
  ...
  const getStatusPill = (status) => {
    switch (status) {
      ...
      case 'waiting_bsn': return <span className="px-2.5 py-1 bg-amber-100 text-amber-800 rounded-full text-xs font-bold tracking-wide">Čeká na BSN</span>;
      case 'active': return <span className="px-2.5 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-bold tracking-wide">Aktivní v NL</span>;
      default: return null;
    }
  };
      ...
      <main className="flex-1 p-8 max-w-[1400px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center gap-4">
             ...
             <div>
               <p className="text-sm font-semibold text-slate-500">Lidé k dispozici (Ready-to-Go)</p>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center gap-4">
             ...
             <div>
               <p className="text-sm font-semibold text-slate-500">Čekající na BSN</p>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center gap-4">
             ...
             <div>
               <p className="text-sm font-semibold text-slate-500">Aktivní v NL</p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col h-[600px]">
          ...
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
            <h2 className="text-lg font-bold text-slate-900">Kandidátský Pipeline</h2>
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Kandidát</th>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Pozice</th>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Věk / Jazyk</th>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Termín BSN</th>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Akce</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredCandidates.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="py-4 px-6 text-sm text-slate-600">{c.age} let / <span className="font-semibold text-slate-900">{c.language}</span></td>
                    ...
                    <td className="py-4 px-6 text-right">
                      ...
                      >
                        {c.status === 'active' ? 'Schváleno' : 'Schválit'}
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-auto p-6 space-y-8">
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="flex-1">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Status záznamu</p>
                    {getStatusPill(selectedCandidate.status)}
                  </div>
                  <div className="w-px h-10 bg-slate-200"></div>
                  <div className="flex-1 text-center">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Pozice</p>
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <CheckCircle2 size={16} className="text-emerald-500" /> Ověřené dokumenty
                  </h3>
                  ...
                  <div className="relative rounded-xl overflow-hidden border border-slate-200 aspect-[1.58/1] bg-slate-100 group">
                    ...
                    {isPhotoBlurred && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/10">
                        ...
                        >
                          <Eye size={16} /> Odkrýt doklad
                        </button>
                        <p className="text-xs text-slate-600 font-medium mt-2 bg-white/80 px-2 py-1 rounded">Jen pro účely prověrky</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Věk</p>
                    <p className="font-bold text-slate-900">{selectedCandidate.age} let</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Jazyk</p>
                    <p className="font-bold text-slate-900">{selectedCandidate.language}</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Termín BSN</p>
                    <p className="font-bold text-slate-900">{selectedCandidate.bsnDate}</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Odjezd</p>
                ...
                {selectedCandidate.hasVideo && (
                  <div>
                    <button className="w-full flex items-center justify-center gap-2 bg-orange-50 text-orange-700 py-3 rounded-xl hover:bg-orange-100 transition-colors border border-orange-200">
                      <Video size={18} /> <span className="font-bold text-sm">Přehrát video-vizitku</span>
                    </button>
                  </div>
                )}
              ...
              <div className="p-6 border-t border-slate-100 bg-white space-y-3">
                ...
                >
                  <FileText size={18} /> Generovat PDF Pack
                </button>
                <button 
                ...
                >
                  {selectedCandidate.status === 'active' ? 'Schváleno k nástupu' : 'Schválit k nástupu'}
                </button>
              </div>
